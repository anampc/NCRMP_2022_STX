...
N. Besemer, A. Palacio, A. Webb, G. Kolodziej, I. Enochs - October 2022
</h5>
...
* The most recent NCRMP Atlantic climate monitoring took place in St. Croix in the US Virgin islands from September 4th to September 9th 2022
...
* Subsurface **temperature** recorders (STRs) were recovered and redeployed at all 4 transects, each one composed by 3-4 depths (Fig. 1). Overall,  more than 4 million temperature observations were collected:
  **PERHAPS REMOVE THIS TEXT AND LEAVE ONLY THE TABLE **
  - E (1m): 311,609 observations 
  - White Shoal (5m): 311,213 observations 
  - Bird Key Reef (15m): 311,560 observations 
  - Black Coral Rock (25m): 165,695 observations 
  
```{r Temp observations}
Cases_Loc_depht<-STR_all %>% count(Transect,
                                  Depth,
                                  sort = F)
Cases_Loc_depht<-as.data.frame(Cases_Loc_depht %>%
                   pivot_wider(names_from = Depth,
                   values_from = n))
Cases_Loc_depht$Total<-rowSums(Cases_Loc_depht[, c(2:5)], na.rm=TRUE)
#write.csv(Cases_Loc_depht, "FLK_results/0_SampleS_Region_zone.csv")

kable(Cases_Loc_depht,
      format.args = list(big.mark = ","),
      caption = "Number of temperture observations collected by transect and depht")
```
...
# 3. Get the PAR data  
  light <- read_csv('Data/ecopar-0392-stx-suite.csv',
        ...
        select(-interval) %>%
  #manual start/stop time of monitoring
   filter(DateTime >= as.POSIXct('2022-09-05 09:20:32', tz='America/Puerto_Rico') &
          DateTime <= as.POSIXct('2022-09-08 10:00:00', tz='America/Puerto_Rico'))
...
* At XXX, short term instruments (72h) were deployed to  monitor daily fluctuations in: 
  - **Current**: 294 observations
  - **pH**: 294 observations
  - **Light**: 290 observations
  - **Carbonate Chemistry**: XX samples collected
  
* Additionally, changes in benthic cover and carbonate production were monitored at XXX by recording:
  - **Bioerosion**: XX Bioerosion Monitoring Units (BMUs) collected, XX redeployed 
  - **Calcification**: XX Calcification Accretions Units (CAUs) collected, XX redeployed
  - **Benthic cover**: XX landscape mosaics
  - **Carbonate budget surveys**: Benthic cover, sponge, urchin and parrotfish surveys completed at XX transects
...
stations$Depth<-ordered(stations$Depth, levels = c("1 m", "5 m", "15 m","25 m"))
...
# Change color depending on type of data?
  ...
  pal.gen2 <- colorFactor(palette = "Blues",
                         domain = stations$Depth)
  # pal.gen3 <- colorFactor(palette = "Set1",
  #                        domain = stations$Depth)
...
```{r makeAmap, echo=FALSE, fig.width = 8, fig.height = 4, fig.align='center'}
...
# Sub_regions
     addRectangles(
      lng1=-64.59760, lat1=17.76236,
      lng2=-64.467045, lat2=17.828715,
      opacity = 1,
      color = "black",
      #color = "#F8766D",
      weight=2,
      ...
      labelOptions = labelOptions(
                       ...
                       textOnly = F))  %>%
    addRectangles(
      lng1=-64.77, lat1=17.782,
      lng2=-64.75, lat2=17.795,
      opacity = 1,
      color = "black",
      weight=2,
      fillColor = "transparent",
      ...
      labelOptions = labelOptions(
                       ...
                       textOnly = F))  %>%
    addRectangles(
     lng1=-64.91, lat1=17.71,
      lng2=-64.891, lat2=17.758,
      opacity = 1,
      color = "black",
      weight=2,
      fillColor = "transparent",
      ...
      labelOptions = labelOptions(
                       ...
                       textOnly = F)) %>%
    addRectangles(
    lng1=-64.67, lat1=17.671,
      lng2=-64.62, lat2=17.72,
      opacity = 1,
      color = "black",
      weight=2,
      fillColor = "transparent",
...
Three years of temperature measurements were retrieved and processed from 4 sites (depths). Temperature was measured using SeaBird Electronics Subsurface Temperature Recorders (STR)s that collected data at 5-minute intervals.
...
<font size="2"> **Figure 2:** Temperature conditions at four transects in Saint Croix (east, west, north and south) representing a depth gradient (1m, 5m, 15mand 25m). CHANGE THIS: Data were collected from November 2018 to June 2021, with the exception of the 25m STR that recorded until February 7th 2020. </font>
...
At XXXXX Salt River Bay additional instruments were deployed for a 72-hour diurnal suite that monitored pH, temperature, light and current speed (Fig. 3). The SeaFET pH logger, EcoPAR and Tiltmeter collected measurements at 15-minute intervals.
...
At XXXXX SITE XX transects were surveyed to obtain carbonate budgets EXPLAIN WHAT THIS IS (Fig. 4). HOW DO WE DO THEM. why do show 2 years... BLa BLA BLA 

```{r, Carbonate_B_plot, echo=FALSE, warning=FALSE, fig.width = 9.5, fig.height = 5.5}
## boxplots production
...
<font size="2">**Figure 4:** EXPLAIN THIS PLOT. </font>
-----
...
##I THINK I WANT TO ADD NPS AND CARIBBEAN SEA ADVENTURES LOGOS IF POSSIBLE##
