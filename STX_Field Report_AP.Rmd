...
* At Salt River Bay, short term instruments (72h) were deployed to  monitor daily fluctuations in: 
  - **Current**: 880 observations
  - **pH**: 879 observations
  - **Light**: 876 observations
  - **Carbonate Chemistry**: 24 samples collected
  
* Additionally, changes in benthic cover and carbonate production were monitored at Salt River Bay by recording:
  - **Bioerosion**: 38 Bioerosion Monitoring Units (BMUs) collected, 40 redeployed 
  - **Calcification**: 5 Calcification Accretions Units (CAUs) collected, 20 redeployed
...
stations$Depth<-ordered(stations$Depth, levels = c("1", "5", "15","25"))
...
```{r plot STR temperature2, echo=FALSE, fig.width = 9.5, fig.height = 4}
plot2<-ggplot(data = STR_all) +
      geom_line(aes(x = ymd_hms(UTCDateTime), 
                    y = Temperature, colour=Transect),
                size=0.3) +
...
box2 <- ggplot(data = STR_all) +
       geom_boxplot(aes(x = Transect, y = Temperature, fill=Transect)) +
       theme_bw() +
       theme(panel.grid = element_blank(),
             axis.text.y = element_blank(),
             #axis.title.x = element_blank(),
...
Figure2_2 <- ggarrange(plot2, box2, widths = c(8/10, 2/10))
Figure2_2
...
STR_all_summary <- STR_all %>%
  group_by(MonthYear, Transect, Depth) %>%
  summarise(
...
# 3. Get the PAR data  
  light <- read_csv('Data/ecopar-0392-stx-suite.csv',
                    col_types = cols()) %>%
          mutate(DateTime = mdy_hms(`Datetime (AST)`, tz="EDT")) %>%
          #mutate(DateTime = parse_date_time(DateTime, '%m/%d/%Y %H:%M:%S')) %>%
              #mutate(DateTime = with_tz(DateTime, 'America/New_York'))%>%
...
pH_Temp_Plot<-ggplot(data = pH) +
  ...
  # Day/Night
  annotate("rect",
          xmin = seq.POSIXt(as.POSIXct('2022-09-05 19:24:00', tz='EST'),
                         by = "day", length.out = 4),
          xmax = seq.POSIXt(as.POSIXct('2022-09-06 05:45:00', tz='EST'),
                         by = "day", length.out = 4),
          ymin = -Inf, ymax = Inf,
          fill="grey", alpha=0.5) +
  
   annotate("text",
          label = paste("Sep", 05:08),
          x = seq.POSIXt(as.POSIXct('2022-09-05 15:00:00', tz='EST'),
                         by = "day", length.out = 4),
          y = 7.97)+ 
  # Data
